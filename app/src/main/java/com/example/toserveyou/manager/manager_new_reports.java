...
public class manager_new_reports extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View v= inflater.inflate(R.layout.fragment_manager_new_reports, container, false);
    ...
    {
        admin_report=v.findViewById(R.id.manager_new_report);
                ...
                if (snapshot.exists()) {
                    for (DataSnapshot snap : snapshot.getChildren()) {
                        if (snap.getKey().equals(user_id)) {
                            database.child("new_reports").child(user_id).addValueEventListener(new ValueEventListener() {
                                @Override
                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                    for (DataSnapshot d : snapshot.getChildren()) {
                                            send_report_statuse report = new send_report_statuse(d.child("date").getValue().toString(), d.child("employee_name").getValue().toString(), d.child("physical_address").getValue().toString(), d.child("problem_description").getValue().toString(),
                                                    ...
                                                    , d.child("time").getValue().toString(), user_id,"", "لم تحل", "لم يتم حل المشكلة");
                                            arrayList.add(report);
                                    }
                                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.GINGERBREAD_MR1) {
                                        Collections.sort(arrayList, Comparator.comparing(send_report_statuse::getReport_number));
                                        Collections.reverse(arrayList);
                                        manager_adapter adapter = new manager_adapter(arrayList, manager_new_reports.this);
